...
/**
 * @author TA
 * @category MDIS send SMS
 */

@Component
@Service
public class MDISController {
	...
	public SendSMS_Res sendSMS(SendSMS_Req req) {
		...
		String hostLoggerName = GlobalConstants.HostLog_MDIS;
		...
		try {
			 ...
			 * @param language
			 *            SMS Body, mandatory and not null
			 * 
			 *            validate input parameter whether mobileNumber,language and SMS body is null
			 *            or empty. if the input is null @return failure message.. otherwise
			 *            flows to be continued.
			 * 
			 ****/

			resObj = validateInput.sendSMS(req);
			...
			properties.put("tpSystem", GlobalConstants.TPSystem_MDIS);
			 ...
			 * name.
			 * 
			 * @return config.properties, operation.properties, MDISpayload.properties and
			...
			req.setHost(GlobalConstants.TPSystem_MDIS);
			...
			resObj = service.sendSMS(inputParams);
